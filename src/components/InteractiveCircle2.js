...
class InteractiveCircle2 extends Component {
  ...
  showItems = () => {
    ...
    const ITEM = dataPage.multimediaCircular.map( (item, i) => {
      return(
        <div className = 'circleItems' key = { i } style = { { 'top': item.pos.top, 'left': item.pos.left } }>
          <button className = { 'circleButton d-Flex j-C aI-C ' + ( i + 1 !== 1 ? 'disabled2' : '')} id = { i + 1 } onClick = { this.enableItem }>
            <img alt='Imagen' className='' src={ item.img } id = { i + 1 } />
          </button>
        </div>
      );
    } );
  ...
  render() {
    ...
    return (
      <div className = 'interactiveCircle2 d-Flex j-Ar aI-C c-7 mL-5'>
        {
          // MOSTRAR LOS GLOBOS DE TEXTO
          this.state.openGlobe !== false ?
          <div className = 'bgItemGlobe'>
            <div
              className = { 'itemGlobe animated d-Flex j-C aI-C ' + (multimediaCircular[actualItem - 1].itemInfo.posTriang)}
              ...
              <h1 className = 'F4-5 color-4 mR-025'>{ multimediaCircular[actualItem - 1].itemInfo.title }</h1>

              <div className = 'scrollText'>
                <p className = 'color-5' dangerouslySetInnerHTML = { { __html: multimediaCircular[actualItem - 1].itemInfo.text } } />
              </div>
              {
