...
const DragWord = ({ name, type, id, countWords, color }) => {
  const [{ isDragging }, drag] = useDrag({
    item: { name, type: type },
      ...
      if (item && dropResult) {
        ...
        document.getElementById('boxDrop-' + type).style.backgroundColor = color;
  ...
  return (
    <div className = 'dragWord' ref = { drag } style = {{ ...style, opacity, 'background': color }} id = {'dragWord-' + id } onDragEnd = { countWords }>
      <p>{ type }</p>
    </div>
  )
}
